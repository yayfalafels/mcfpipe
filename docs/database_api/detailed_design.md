...
All routes and behaviors are declared in config/routes.json. Below is the default set.
...
| create-item | POST | `/{table}` | Table.create(body) | JSON body | 201 {"pk":…} |
| put-item | PUT | `/{table}/{id}` | Table.put(id, body) | JSON body | 200 {"pk":…} |
| delete-item | DELETE | `/{table}/{id}` | Table.delete(id) | – | 200 {"pk":…} |
| batch-write | POST | `/{table}/batch` | Table.batch_write(items) | `{"items":[...]}` or [...] | 200 `{"ok":N,"failed":[...]}` |
| batch-delete | POST | `/{table}/delete` | Table.batch_delete(keys) | `{"keys":[{pk,sk?},...]}` | 200 {"ok":N,"failed":[...]} |
...
| health | GET | `/__admin/health` | ping | – | 200 `{"ok":true}` |

### 3.1 Search Query Parameters (default mapping)
...
```json
{
  ...
  "message": "Missing required field: pk",
...
- **API Gateway (private)**: attach to VPC Endpoint; apply resource policy restricting to vpce-*.
- **CloudWatch Logs**: Lambda logs auto-ingested; ensure a log retention policy (e.g., 14–30 days).
- **Config rollout**: write new routes.json/policies.json to S3, call /__admin/refresh.
...
## 19 Appendix — Sample Events and Responses
...
{"job_id":"J123","title":"DE","company":"RSK"}
```
